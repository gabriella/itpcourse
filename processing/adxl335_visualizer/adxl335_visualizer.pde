...
void setup(){
  ...
  font = createFont(PFont.list()[269], 24);
  smallFont();
...
void draw()
{
  ...
  drawGraph(xAxis, 100, color(#ff34ff), "X - Axis");  
  drawGraph(yAxis, 200, color(#000000), "Y - Axis");
  drawGraph(zAxis, 300, color(#cccccc), "Z - Axis");
...
void drawGraph(int[] arrToDraw, int yPos, color graphColor, String name){
  ...
  text(name + " : " + gString + "G", 10, yPos - 10);
  popStyle();
}
...
void draw3d(int currentX, int currentY, int currentZ){
  ...
  float finalZ = map(normalizedZ, 0.0, 1.0, 300.00, -100.0);
